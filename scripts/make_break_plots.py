...
def plot_timeseries_with_means(df: pd.DataFrame, value_col: str, ylabel: str, title: str, subtitle: str, outpath: Path):
    ...
    ax.axhline(pre_mean, color="#ff7f0e", linestyle="-", linewidth=1.4, label=f"Mean {PRE_START}-{PRE_END}")
    ax.axhline(post_mean, color="#2ca02c", linestyle="-", linewidth=1.4, label=f"Mean {POST_START}-{POST_END}")
...
def plot_distribution(pre: pd.Series, post: pd.Series, ylabel: str, title: str, outpath: Path):
    fig, ax = plt.subplots(figsize=(7.5, 5))
    data = [pre * 100.0, post * 100.0]
    labels = [f"{PRE_START}-{PRE_END}", f"{POST_START}-{POST_END}"]
    positions = [0, 1]
    ...
    for pos, vals in zip(positions, data):
        ...
        high *= 100.0 / 100.0
        ax.scatter(pos, mean_val, color="#d62728", marker="D", s=50, zorder=4, label=None)
        ax.vlines(pos, low, high, colors="#d62728", linewidth=2)
